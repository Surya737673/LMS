...
export const editLayout =(async (req:Request,res:Response,next:NextFunction)=>{
    try{    
        ...
        if(type === "Categories"){
            const { categories } = req.body
            const CategoriesDate = await LayoutModel.findOne({type:"Categories"})
            const categoriesItems = await Promise.all(
                categories.map(async(item:any)=>{
                    console.log(item)
                    return{
            ...
            await LayoutModel.findByIdAndUpdate(CategoriesDate?._id, {type:"Categories", categories: categoriesItems})
        }
